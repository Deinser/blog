...
from flask import render_template,session,url_for,redirect,abort,flash,request,abort,make_response,current_app
from flask_login import current_user,login_required

from . import main
from .forms import NameForm,EditProfileForm,EditProfileAdminForm,PostForm,CommentForm
from ..models import User ,Post,Follow,Permission,Role,Comment
from .. import db
...
def index():
	...
	if form.validate_on_submit() and current_user.can(Permission.WRITE_ARTICLES):
		...
		post.author=current_user._get_current_object()  #数据库真正的用户对象，current_user是个代理
		db.session.add(post) 							#对象，用_get_current_object()方法获取真实对象
		flash('提交成功')
...
def user(username):
	...
	if user is None:
		abort(404)
	p=Post.query.filter_by(author=user).all()
...
def followed_by(username):
	...
	pagination=user.followed.order_by(Follow.timestamp.desc()).paginate(
	           page,per_page=15,
			   error_out=False)
	follows=[{'user':item.followed,'timestamp':item.timestamp} for  
	            item in pagination.items]
	return render_template('followed_by.html',user=user,title="Followed of",
